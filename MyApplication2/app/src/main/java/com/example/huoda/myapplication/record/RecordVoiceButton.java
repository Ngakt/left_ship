...
import com.example.huoda.myapplication.listener.RecordVoiceListener;
import com.example.huoda.myapplication.record.RecordVoiceBtnStyle;
...
public class RecordVoiceButton extends ImageButton {
    ...
    private com.example.huoda.myapplication.record.RecordVoiceBtnStyle mStyle;
...
//                this.setText(mStyle.getTapDownText());
                ...
                if (isSdCardExist()) {
                    ...
                    Toast.makeText(this.getContext(), mContext.getString(cn.jiguang.imui.chatinput.R.string.sdcard_not_exist_toast),
...
//                break;
    ...
    public void finishRecord(boolean isPreview) {
        ...
        if (intervalTime < MIN_INTERVAL_TIME) {
            Toast.makeText(getContext(), mContext.getString(cn.jiguang.imui.chatinput.R.string.time_too_short_toast), Toast.LENGTH_SHORT).show();
            ...
            if (myRecAudioFile != null && myRecAudioFile.exists()) {
                ...
                if (mp != null) {
                    ...
                    Toast.makeText(mContext, mContext.getString(cn.jiguang.imui.chatinput.R.string.record_voice_permission_request),
                            Toast.LENGTH_SHORT).show();
                }
            }
        }
    ...
    private void startRecording() {
        try {
            ...
            e.printStackTrace();
            Toast.makeText(mContext, mContext.getString(cn.jiguang.imui.chatinput.R.string.illegal_state_toast), Toast.LENGTH_SHORT).show();
            cancelTimer();
            dismissDialog();
            if (mThread != null) {
                mThread.exit();
            ...
            Toast.makeText(mContext, mContext.getString(cn.jiguang.imui.chatinput.R.string.record_voice_permission_denied),
                    Toast.LENGTH_SHORT).show();
            cancelTimer();
            dismissDialog();
            if (mThread != null) {
...
//        this.setText(mStyle.getVoiceBtnText());
    ...
    private static class ShowVolumeHandler extends Handler {
        ...
        private final WeakReference<cn.jiguang.imui.chatinput.record.RecordVoiceButton> lButton;

        public ShowVolumeHandler(cn.jiguang.imui.chatinput.record.RecordVoiceButton button) {
            lButton = new WeakReference<cn.jiguang.imui.chatinput.record.RecordVoiceButton>(button);
        }

        @Override
        public void handleMessage(android.os.Message msg) {
            cn.jiguang.imui.chatinput.record.RecordVoiceButton controller = lButton.get();
            if (controller != null) {
                //TODO show wave view
            }
        }
    ...
    private static class MyHandler extends Handler {
        private final WeakReference<com.example.huoda.myapplication.record.RecordVoiceButton> lButton;

        public MyHandler(com.example.huoda.myapplication.record.RecordVoiceButton button) {
            lButton = new WeakReference<cn.jiguang.imui.chatinput.record.RecordVoiceButton>(button);
        }
